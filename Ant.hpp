	...
	public:
		...
		virtual inline uint64_t NextMove(uint64_t repetition)
		{
			for (uint64_t i = repetition; i; --i)
			{
				...
				uint8_t temp = m_pTurnsFromColor[m_CurrentAntColorMasked] | m_CurrentAntColorMasked;

				m_pMeshFieldCopy[m_y * m_Width + m_x] = temp;
				m_pMesh->m_pfieldVertex->operator[](m_y* m_Width + m_x).color = sf::Color(0U, m_pDaGreenColorAndTurns[m_CurrentAntColorMasked].g, 0U);
