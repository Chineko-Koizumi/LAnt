...
da::Mesh::Mesh(uint32_t width, uint32_t height, sf::RenderWindow* window, uint16_t AdditionalNumber)
	:m_FieldWidth(width)
	,m_FieldHeight(height)
	,m_pWindow(window)
	,m_pVertexAccesPointer(nullptr)
	,m_AdditionalNumberForFileName(AdditionalNumber)
{
...
void da::Mesh::DumpToFileAndExit()
{
	DrawMesh();
	m_pWindow->display();
	DrawMesh();
	...
	std::string FileName("SS" + std::to_string(m_AdditionalNumberForFileName) + ".png");
...
void da::Ant::CheckBounds()
{
	...
	{
		std::cout << std::endl << "m_x > m_Width || m_x < 0" << std::endl;
		m_pMesh->DumpToFileAndExit();
	}
	if(m_y > m_Height - 1 || m_y < 0)
	{
		std::cout << std::endl << "m_y > m_Height || m_y < 0" << std::endl;
		m_pMesh->DumpToFileAndExit();
	}
}
